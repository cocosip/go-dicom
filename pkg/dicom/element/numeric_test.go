...
func TestFloat(t *testing.T) {
	...
	t.Run("GetValue", func(t *testing.T) {
		for i, want := range values {
			...
			if got != want {
				t.Errorf("GetValue(%d) = %f, want %f", i, got, want)
			}
		}
	})
	...
	t.Run("GetValues", func(t *testing.T) {
		...
		for i, want := range values {
			if got[i] != want {
				t.Errorf("GetValues()[%d] = %f, want %f", i, got[i], want)
			}
		}
	})
}
