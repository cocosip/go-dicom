...
// Package endian provides byte-order representation and swap utilities.
package endian
...
// Endian represents the byte order of binary data.
type Endian int
...
// Native returns the byte order of the local machine.
func Native() Endian {
	return native
}
...
// CopyAndSwap returns a copy of data with each complete group reversed.
func CopyAndSwap(bytesToSwap int, data []byte) []byte {
	result := append([]byte(nil), data...)
	SwapBytes(bytesToSwap, result)
	return result
}

// ConvertEndianness returns an independently owned copy with every complete
// 2-, 4-, or 8-byte sample reversed. Unlike CopyAndSwap, it rejects an
// unsupported sample size or a partial trailing sample.
func ConvertEndianness(data []byte, bytesPerSample int) ([]byte, error) {
	...
	if len(data)%bytesPerSample != 0 {
		return nil, fmt.Errorf("data length %d not divisible by bytes per sample %d", len(data), bytesPerSample)
	}
	result := make([]byte, len(data))
	copy(result, data)
	SwapBytes(bytesPerSample, result)
	return result, nil
}
