...
type OutputLUT struct {
	colorMap [256]colorconv.Color32
}
...
func NewOutputLUT(colorMap [256]colorconv.Color32) *OutputLUT {
	return &OutputLUT{
		colorMap: colorMap,
	}
}
...
func NewOutputLUTFromValues(first float64, values []uint16) *OutputLUT {
	...
	for i := 0; i < 256; i++ {
		...
		colorMap[i] = colorconv.Color32{A: 255, R: v, G: v, B: v}
	}
...
func (o *OutputLUT) GetColor(value int) colorconv.Color32 {
	if value < 0 {
		return o.colorMap[0]
	}
	if value > 255 {
...
func (o *OutputLUT) ColorMap() [256]colorconv.Color32 {
	return o.colorMap
}
